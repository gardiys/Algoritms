...
def dijkstra(g, w, n, s):
    ...
    q = set(range(1, n+1))  # Заносим все вершины во множество q

    while len(q) > 0:  # Пока множество q непустое выполняем следующие действия
        min_u = next(iter(q))  # Это короткий способ взять один элемент из множества

        # В цикле ищем вершину, у которой наименьшее значение кратчайшего расстояния
        for u in q:
        ...
        # Для каждой вершины, которая смежная с минимальной, вызываем процедуру Relax
